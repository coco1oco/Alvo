    ...
    <div class="view">
        ...
        <div class="view-header">
            <div>
                ...
                <p class="view-subtitle">
                    Set financial targets, track progress, and reach your
                    milestones
                </p>
            </div>
        ...
        <template v-if="loading">
            <div class="glass-card p-6 sk-card mb-6">
                ...
                <div
                    class="skeleton"
                    style="width: 100%; height: 3.5rem; border-radius: 0.75rem"
                ></div>
            </div>
        </template>
                ...
                <div class="summary-hero-content">
                    <div>
                        <span class="summary-label"
                            >Total Saved Across Goals</span
                        >
                        <h2 class="summary-value tabular-nums amount-positive">
                    ...
                    <div class="summary-breakdown">
                        <div class="summary-box">
                            ...
                            <span
                                class="box-value tabular-nums text-primary-color"
                                >{{ goals.length }}</span
                            >
                        </div>
                        <div class="summary-box">
            ...
            <div class="mb-8">
                ...
                <div v-else class="goals-grid">
                    ...
                    >
                        ...
                        <div>
                            ...
                            <div class="flex items-center justify-between">
                                <div class="flex items-center gap-3">
                                    ...
                                    <div>
                                        <h3
                                            class="font-bold text-sm text-primary-color"
                                        >
                                            {{ goal.name }}
                                        </h3>
                                        <p
                                            v-if="goal.linked_account"
                            ...
                            <div class="mt-4">
                                ...
                                >
                                    <span
                                        class="text-2xl font-extrabold tabular-nums amount-positive"
                                    >
                                        {{
                                            formatCurrency(
                                                goal.effective_current_amount,
                                            )
                                        }}
                                    </span>
                                    <span
                            ...
                            <div
                                class="mt-3 text-xs text-muted flex flex-wrap justify-between gap-1 border-t border-border/40 pt-2"
                            >
                                <span v-if="goal.deadline"
                                    >Deadline:
                                    {{ formatDate(goal.deadline) }}</span
                                >
                        ...
                        <div
                            class="mt-4 pt-3 flex gap-2 border-t border-border/40"
                        >
                            <button
            ...
            <div class="modal-panel">
                ...
                <form @submit.prevent="saveGoal" class="modal-form">
                    ...
                    <div>
                        ...
                        >
                            <option value="">
                                No linked account (manual tracking)
                            </option>
                            <option
                        ...
                        <p class="field-hint">
                            Linking an account automatically uses its balance to
                    ...
                    <div class="form-row-2">
                        ...
                        <div>
                            ...
                            <input
                                v-model="form.color"
                                type="color"
                                class="input-field h-10 p-1 cursor-pointer"
                            />
                        </div>
                    </div>
        ...
        <div v-if="showDepositModal" class="modal-overlay">
            <div class="modal-panel">
                <div class="modal-header">
                    <h3 class="modal-title">
                        Add Deposit to "{{ depositGoal?.name }}"
                    </h3>
                    <button
                ...
                <form @submit.prevent="submitDeposit" class="modal-form">
                    <div>
                        ...
                        <select
                            v-model="depositFromAccountId"
                            required
                            class="input-field"
                        >
                            <option value="">Select account...</option>
                            <option
                                v-for="acc in depositSourceAccounts"
                                :key="acc.id"
                                :value="acc.id"
                            >
                        ...
                        <p class="field-hint">
                            The transfer will post into the goal's linked
                            savings account.
                        </p>
                    </div>
                    ...
                    <div>
                        <label class="label"
                            >Deposit Amount ({{ getCurrencySymbol() }})</label
                        >
                        <input
                    ...
                    <div v-if="depositError" class="alert-danger">
                        {{ depositError }}
                    </div>
...
const depositFromAccountId = ref("");
...
const totalTarget = computed(() =>
    goals.value.reduce((s, g) => s + (parseFloat(g.target_amount) || 0), 0),
);
const totalSaved = computed(() =>
    goals.value.reduce(
        (s, g) => s + (parseFloat(g.effective_current_amount) || 0),
        0,
    ),
);
const completedGoals = computed(() =>
    goals.value.filter((g) => g.progress_percentage >= 100),
);
...
const depositSourceAccounts = computed(() =>
    accounts.value.filter(
        (account) =>
            account.type !== "credit_card" &&
            account.id !== depositGoal.value?.linked_account_id,
    ),
);
...
function openDepositModal(goal) {
    ...
    depositAmount.value = "";
    depositFromAccountId.value = depositSourceAccounts.value[0]?.id ?? "";
    depositError.value = "";
...
async function submitDeposit() {
    ...
    try {
        await axios.post(`/api/goals/${depositGoal.value.id}/deposit`, {
            amount: parseFloat(depositAmount.value) || 0,
            from_account_id: depositFromAccountId.value,
        });
        ...
        depositError.value =
            e.response?.data?.message || "Failed to add deposit";
        toast(depositError.value, "error");
    } finally {
