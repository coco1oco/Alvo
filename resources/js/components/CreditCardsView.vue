  ...
  <div class="view">
    ...
    <div class="view-header">
      <div>
        ...
        <p class="view-subtitle">Track card balances, credit limits, billing cycles, and pay bills</p>
      </div>
      ...
      <div class="glass-card summary-hero-card mb-6 sk-card">
        <div class="summary-hero-content">
          <div style="display:flex;flex-direction:column;gap:0.5rem">
            ...
            <div class="skeleton" style="width:12rem;height:2rem;border-radius:0.5rem"></div>
            <div class="skeleton" style="width:10rem;height:0.75rem;border-radius:0.375rem"></div>
          </div>
    ...
    <template v-else>
      ...
      <div class="glass-card summary-hero-card mb-6">
        <div class="summary-hero-content">
          <div>
            ...
            <h2 class="summary-value tabular-nums amount-negative">{{ formatCurrency(totalOwed) }}</h2>
            <p class="summary-subtext">Across {{ creditCardAccounts.length }} credit card{{ creditCardAccounts.length === 1 ? '' : 's' }}</p>
          </div>

          <div class="summary-breakdown">
            <div class="summary-box">
              <span class="box-label">Total Credit Limit</span>
              <span class="box-value tabular-nums">{{ formatCurrency(totalLimit) }}</span>
            </div>
            <div class="summary-box">
              <span class="box-label">Available Credit</span>
              <span class="box-value amount-positive tabular-nums">{{ formatCurrency(totalAvailable) }}</span>
            </div>
            <div class="summary-box">
              <span class="box-label">Overall Utilization</span>
              <span class="box-value tabular-nums" :class="getUtilizationTextColor(overallUtilization)">
                {{ overallUtilization }}%
              </span>
            </div>
          </div>
        ...
        <div v-if="totalLimit > 0" class="mt-4">
          <div class="flex justify-between text-xs font-semibold text-muted mb-1.5">
            <span>Overall Credit Used: {{ overallUtilization }}%</span>
            <span>Available: {{ formatCurrency(totalAvailable) }}</span>
          </div>
          <div class="overall-progress-bar">
            <div class="overall-progress-fill"
                 :class="getUtilizationBarClass(overallUtilization)"
                 :style="{ width: overallUtilization + '%' }"></div>
          </div>
        </div>
      </div>
        ...
        <div class="due-schedule-card glass-card">
          <div class="due-schedule-header">
            <div class="flex items-center gap-2">
              ...
              <h3 class="text-sm font-bold text-primary-color">Upcoming Bill Statements & Due Dates</h3>
            </div>
          </div>
          <div class="due-schedule-grid">
            <div v-for="item in upcomingDueCards" :key="item.card.id" class="due-card-item">
              <div class="flex items-center gap-2.5">
                <div class="w-7 h-7 rounded-lg flex items-center justify-center flex-shrink-0" :style="{ backgroundColor: item.card.color + '25', color: item.card.color }">
                  <img v-if="item.card.icon && item.card.icon !== 'wallet'" :src="`/bankIcons/${item.card.icon}`" class="w-4 h-4 object-contain" />
                  <svg v-else class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 10h18M7 15h1m4 0h1m-7 4h12a3 3 0 003-3V8a3 3 0 00-3-3H6a3 3 0 00-3 3v8a3 3 0 003 3z" /></svg>
                </div>
                <div>
                  <h4 class="text-xs font-bold text-primary-color">{{ item.card.name }}</h4>
                  <p class="text-[11px] text-muted font-medium">Owed: {{ formatCurrency(item.card.balance) }}</p>
                </div>
              </div>

              <div class="flex items-center gap-3">
                <div class="text-right">
                  <span v-if="item.card.due_date_day" class="text-xs font-semibold block text-primary-color">Due {{ ordinal(item.card.due_date_day) }}</span>
                  <span v-if="item.card.billing_cycle_day" class="text-[10px] text-muted block">Statement {{ ordinal(item.card.billing_cycle_day) }}</span>
                </div>
                <button @click="openPayBill(item.card)" class="btn-paybill-sm">
                  Pay Bill
                </button>
              </div>
            </div>
      ...
      <!-- Credit Cards Grid -->
      <div class="mb-8">
        <h2 class="text-lg font-bold text-primary-color mb-4">Your Cards</h2>
        
        <div v-if="!creditCardAccounts.length" class="empty-state glass-card">
          <svg class="empty-icon" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M3 10h18M7 15h1m4 0h1m-7 4h12a3 3 0 003-3V8a3 3 0 00-3-3H6a3 3 0 00-3 3v8a3 3 0 003 3z" />
          </svg>
          <p class="empty-text">No credit cards added yet.</p>
          <button @click="openModal()" class="btn-primary mt-3">Add Your First Credit Card</button>
        </div>
        ...
        <div v-else class="accounts-grid">
          ...
          >
            ...
            <div class="account-top flex items-center justify-between">
              <div class="account-identity flex items-center gap-3">
                ...
                <div>
                  ...
                  <p class="account-subtag text-xs text-muted font-medium">Credit • PHP</p>
                </div>
              </div>

              <!-- Menu Button Dropdown -->
              <div class="relative">
                <button @click.stop="toggleAccountMenu(acc.id)" class="menu-dots-btn" title="Options">
                  •••
                </button>

                <div v-if="activeMenuId === acc.id" class="account-dropdown-menu" @click.stop>
                  <button @click="openPayBill(acc); activeMenuId = null" class="dropdown-item dropdown-item--paybill">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 10h18M7 15h1m4 0h1m-7 4h12a3 3 0 003-3V8a3 3 0 00-3-3H6a3 3 0 00-3 3v8a3 3 0 003 3z" />
                    </svg>
                    Pay Bill
            ...
            <div class="account-balance-section mt-4">
              <p class="balance-label uppercase text-[10px] tracking-wider font-bold text-muted">Used Credit (Owed)</p>
              <p class="balance-value tabular-nums font-extrabold text-2xl amount-negative">
            ...
            <div class="credit-gauge-section mt-3">
              <div class="flex justify-between text-xs font-semibold mb-1" :class="getUtilizationTextColor(getCardUtilization(acc))">
                <span>{{ getCardUtilization(acc) }}% used</span>
                <span>{{ formatCurrency(getCardAvailable(acc)) }} left</span>
              </div>
              <div class="proportion-bar-wrapper">
                <div class="proportion-bar"
                     :class="getUtilizationBarClass(getCardUtilization(acc))"
                     :style="{ width: getCardUtilization(acc) + '%' }"></div>
              </div>

              <!-- Badges -->
              <div v-if="getCardUtilization(acc) >= 75" class="credit-warning-badge mt-2">
                ...
                {{ getCardUtilization(acc) >= 100 ? 'Limit Reached' : 'High Utilization (>75%)' }}
              </div>

              <!-- Billing Info -->
              <div class="credit-limit-display mt-2 flex flex-wrap gap-x-2 gap-y-1 items-center">
                <span>Limit: <strong>{{ formatCurrency(acc.credit_limit || 0) }}</strong></span>
                <span v-if="acc.billing_cycle_day">· Statement: {{ ordinal(acc.billing_cycle_day) }}</span>
                <span v-if="acc.due_date_day">· Due: {{ ordinal(acc.due_date_day) }}</span>
              </div>
            </div>

            <!-- Card Actions Footer -->
            <div class="card-action-bar mt-4 pt-3 flex gap-2 border-t border-border/50">
              <button @click="openPayBill(acc)" class="btn-primary flex-1 text-xs py-2 justify-center">
                Pay Bill
              </button>
              <button @click="openTransaction(acc)" class="btn-ghost flex-1 text-xs py-2 justify-center">
                + Charge
              </button>
            </div>
          </div>
      ...
      <div class="mb-8">
        <div class="flex items-center justify-between mb-4">
          <div>
            <h2 class="text-lg font-bold text-primary-color">Recent Card Activity</h2>
            <p class="text-xs text-muted">Charges, refunds, and bill payments across your credit cards</p>
          </div>
        </div>

        <div class="table-card glass-card p-0 overflow-hidden">
          <div v-if="loadingTxns" class="p-6 text-center text-xs text-muted">Loading card activity...</div>
          <table v-else-if="cardTransactions.length" class="w-full text-left border-collapse">
            <thead>
            ...
            <tbody class="divide-y divide-border/40 text-xs">
              <tr v-for="txn in cardTransactions" :key="txn.id" class="hover:bg-bg-surface-2/50 transition-colors">
                <td class="p-3.5 text-muted">{{ formatDate(txn.date) }}</td>
                <td class="p-3.5 font-semibold text-primary-color">
                  <span v-if="txn.account?.type === 'credit_card'">{{ txn.account?.name }}</span>
                  <span v-else-if="txn.to_account?.type === 'credit_card'">{{ txn.to_account?.name }}</span>
                </td>
                <td class="p-3.5">
                  <span v-if="txn.type === 'transfer' && txn.to_account?.type === 'credit_card'" class="badge badge-primary">
                    Pay Bill (from {{ txn.account?.name }})
                  </span>
                  <span v-else-if="txn.type === 'expense'" class="badge badge-danger">Charge</span>
                  <span v-else class="badge badge-success">Refund</span>
                </td>
                <td class="p-3.5 text-secondary-color">
                  <span>{{ txn.category?.name || (txn.description || 'Credit Card Activity') }}</span>
                </td>
                <td class="p-3.5 text-right font-bold tabular-nums"
                    :class="txn.type === 'transfer' && txn.to_account?.type === 'credit_card' ? 'amount-positive' : 'amount-negative'">
                  {{ txn.type === 'transfer' && txn.to_account?.type === 'credit_card' ? '−' : '+' }}{{ formatCurrency(txn.amount) }}
                </td>
              </tr>
            </tbody>
          </table>
    ...
    <div v-if="showModal" class="modal-overlay">
      <div class="modal-panel">
        <div class="modal-header">
          <h3 class="modal-title">{{ editingCard ? 'Edit Credit Card' : 'Add Credit Card' }}</h3>
        ...
        <form @submit.prevent="saveCard" class="modal-form">
          <div>
            ...
            <input v-model="form.name" required placeholder="e.g. BPI Gold Rewards" class="input-field" />
          </div>
          ...
          <div class="form-row-2">
            <div>
              ...
              <input v-model="form.billing_cycle_day" type="number" min="1" max="28" placeholder="e.g. 25" class="input-field" />
              <p class="field-hint">Day of month statement cuts.</p>
            </div>
            <div>
              <label class="label">Payment Due Day <span class="label-optional">(optional)</span></label>
              <input v-model="form.due_date_day" type="number" min="1" max="28" placeholder="e.g. 15" class="input-field" />
          ...
          <div>
            <div class="color-picker-header">
              <label class="label label-no-mb">Card Color & Brand</label>
...
const creditCardAccounts = computed(() => accounts.value.filter(a => a.type === 'credit_card' && !a.is_archived))

const totalLimit = computed(() => creditCardAccounts.value.reduce((sum, a) => sum + (parseFloat(a.credit_limit) || 0), 0))
const totalOwed = computed(() => creditCardAccounts.value.reduce((sum, a) => sum + Math.max(parseFloat(a.balance) || 0, 0), 0))
const totalAvailable = computed(() => Math.max(totalLimit.value - totalOwed.value, 0))

const overallUtilization = computed(() => {
  if (!totalLimit.value) return 0
  return Math.min(Math.round((totalOwed.value / totalLimit.value) * 100), 100)
})

const upcomingDueCards = computed(() => {
  return creditCardAccounts.value
    .filter(card => (card.due_date_day || card.billing_cycle_day) && parseFloat(card.balance) > 0)
    .map(card => ({ card }))
})
...
async function fetchCardTransactions() {
  ...
  try {
    const { data } = await axios.get('/api/transactions', { params: { type: 'credit_card' } })
...
.dropdown-item--paybill { color: var(--primary); }
.dropdown-item--paybill:hover { background-color: var(--primary-light); }
...
.empty-icon { width: 3rem; height: 3rem; color: var(--text-muted); margin-bottom: 0.75rem; }
.empty-text { font-size: 0.875rem; color: var(--text-muted); font-weight: 500; }
...
.input-prefix { position: absolute; left: 0.875rem; font-size: 0.875rem; font-weight: 600; color: var(--text-muted); }
.input-field--prefix { padding-left: 2rem; }

.field-hint { font-size: 0.6875rem; color: var(--text-muted); margin: 0.25rem 0 0; }
.label-required { color: var(--danger); margin-left: 0.125rem; }

.color-picker-header { display: flex; align-items: center; justify-content: space-between; margin-bottom: 0.375rem; }
.color-toggle-btn { background: none; border: none; font-size: 0.75rem; font-weight: 600; color: var(--primary); cursor: pointer; }
.color-picker { display: flex; flex-wrap: wrap; gap: 0.5rem; margin-top: 0.5rem; }
.color-swatch { width: 24px; height: 24px; border-radius: 50%; border: 2px solid transparent; cursor: pointer; transition: transform 0.15s; }
