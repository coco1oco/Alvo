<template>
    <div class="view">
        ...
        <div class="view-header">
            <div>
                ...
                <p class="view-subtitle">
                    Manage your income, expenses &amp; transfers
                </p>
            </div>
        ...
        <div class="table-card">
            ...
            <table v-else class="txn-table">
                ...
                <tbody>
                    ...
                    >
                        ...
                        <td class="td">
                            <span :class="txnBadgeClass(txn)">{{
                                getTxnLabel(txn)
                            }}</span>
                            <span
                                v-if="txn.is_split"
                                class="split-pill"
                                :title="splitTooltip(txn)"
                                >Split</span
                            >
                            <span
                                v-if="txn.is_reimbursable"
                                class="reimbursable-pill"
                        ...
                        <td class="td td--secondary">
                            <div class="flex items-center gap-1.5 flex-wrap">
                                ...
                                <span v-if="txn.to_account" class="td-arrow">
                                    → {{ txn.to_account?.name }}</span
                                >
                                <span
                                    v-if="isCreditCardTxn(txn)"
                                    class="cc-tag"
                        ...
                        <td class="td td--muted td--truncate">
                            <div class="flex items-center gap-1.5">
                                <span>{{ txn.description || "—" }}</span>
                                <button
                                    v-if="txn.attachment_path"
                            ...
                            <small v-if="txn.notes" class="block text-xs text-muted truncate max-w-xs">{{ txn.notes }}</small>
                        </td>
                        <td class="td td--right">
                            <span
                                class="txn-amount tabular-nums"
                                :class="
                                    txn.type === 'income'
                                        ? 'amount-positive'
                                        : txn.type === 'expense'
            ...
            <div class="pagination flex items-center justify-between">
                ...
                <div v-if="pagination.last_page > 1" class="pagination-btns">
                    ...
                    <span
                        class="px-2 py-1 text-xs text-muted font-medium self-center"
                    >
        ...
        <div v-if="showReceiptModal && selectedReceiptTxn" class="modal-overlay" @click.self="showReceiptModal = false">
            <div class="modal-panel max-w-lg p-5">
                <div class="flex items-center justify-between mb-4">
                    <div>
                        <h3 class="font-bold text-base text-primary-color">{{ selectedReceiptTxn.description || 'Receipt Attachment' }}</h3>
                        <p class="text-xs text-muted">{{ formatDate(selectedReceiptTxn.date) }} • {{ formatCurrency(selectedReceiptTxn.amount) }}</p>
                    </div>
                ...
                <div class="receipt-lightbox-content">
                    <img
                        v-if="isImage(selectedReceiptTxn.attachment_path)"
                        :src="selectedReceiptTxn.attachment_path"
                        alt="Receipt Screenshot"
                        class="w-full max-h-[70vh] object-contain rounded-xl border border-border"
                    />
                    <div v-else class="p-8 text-center bg-surface-2 rounded-xl border border-border">
                        <p class="text-sm font-medium mb-3">PDF Attachment</p>
                        <a :href="selectedReceiptTxn.attachment_path" target="_blank" class="btn-primary text-xs inline-flex items-center gap-1.5">
                            Open PDF Receipt ↗
                        </a>
                    </div>
                </div>
                ...
                <div class="mt-4 flex justify-between items-center text-xs text-muted">
                    <a :href="'/api/transactions/' + selectedReceiptTxn.id + '/attachment'" target="_blank" class="text-primary hover:underline font-medium">
                        Open direct image link ↗
                    </a>
                    <button @click="showReceiptModal = false" class="btn-ghost text-xs">Close</button>
                </div>
            </div>
        </div>
    </div>
...
const selectedReceiptTxn = ref(null);
const editingTxn = ref(null);
let searchTimeout = null;

function openReceiptModal(txn) {
    selectedReceiptTxn.value = txn;
...
function splitTooltip(txn) {
    const participants = txn.split_data?.participants || [];
    if (!participants.length) return "Split expense";
    const settledCount = participants.filter((p) => p.is_settled).length;
    return `${settledCount}/${participants.length} settled`;
}
...
async function fetchTransactions() {
    ...
    try {
        ...
        if (filters.reimbursable) params.reimbursable = filters.reimbursable;
